    ...
    for c in courses:
        if d in coverage[c]:
            ...
            if d in PRIORITY_DRIVERS:
                n = sum([ 1 if d2 in coverage[c] else 0 for d2 in OPTIMAL_DRIVERS.intersection(OPTIMAL_DRIVERS) ])
                if n == 1:
                    unique_courses.add(c)
    x = 1
    if len(unique_courses) > 0:
        row = {'driver' : d, 'cost':costs[d]+1, 'unique_coverage': len(unique_courses),
                'value': (costs[d]+1)/len(unique_courses),'unique_courses': str(unique_courses)}
        output.append(row)
